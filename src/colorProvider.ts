import { Color, TextDocument, ColorInformation, Range, ColorPresentation } from "vscode";
...
export const colorProvider = {
	provideColorPresentations(color: Color) {
		let parsedColor = rgb(color.red * 255, color.green * 255, color.blue * 255, color.alpha);
		...
		const alphaForHex = color.alpha !== 1 ? (((color.alpha * 100 * 255) / 100) | 0).toString(16) : "";
		return [
	...
	provideDocumentColors(document: TextDocument) {
		...
		for (let i = 0; i < document.lineCount; i++) {
			...
			while ((match = totalRegex.exec(element.text))) {
				...
				if (color) {
					colors.push(
						new ColorInformation(new Range(i, match.index, i, match.index + match[0].length), color)
					);
				}
			}
		}
